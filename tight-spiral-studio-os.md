...
*Last integrated: 2026-06-30 — TWO LOCKS, two prunes (twin-rule honored). LOCK 1, THE FOUNDER & THE EDGE NAMED (§2): the founder is at root a **systems thinker, educator, humanist, and writing-craft person**; the distinctive **edge** those produce is **assessment/calibration/norming design**. The games are delivery, the calibration is the edge underneath — and the four roots are never a costume over it. LOCK 2, THE PUBLISHING LAYER (§6 close-out): the studio now has a live GitHub Pages site (`walshero.github.io/TIGHT-SPIRAL-STUDIOS/`) — a third durability tier above scratch and shelf; Claude pushes, the founder doesn't hand-manage git; the site is the door-opener (peers, grants, network run on a link, not a description); standing single-file-vs-co-located-asset fork flagged for art-wire time. PRUNE TWINS: (1) `founder-compass.html` retired (superseded by the Bridge); (2) the dead duplicate OS file `tight-spiral-studio-os__6_.md` dropped from the shelf. Both logged §10. — Prior: 2026-06-29 — the PROBE SWEEP + ACCUMULATOR DECAY RULE (§5.4.5.1, decision log §10). A founder structural/diagnostic question is now a system-wide probe: the Conductor extracts the underlying test and runs it against every OS component before answering the narrow case (the question-side twin of Maximize-or-Check), and auto-queues every surfaced gap for graduate-or-kill so a probe is never a write-only note. Linked call: every accumulator (parking lot, memory, panel roster, Props Room, file shelf) must be able to shed — last-touched stamp + why-held note, review outcomes are promote-or-kill only, "held" has a clock. The Conductor itself was written into the OS here for the first time (memory-resident since 2026-06-27 but never on the durable shelf — a caught accumulator gap). Worked example: one question aimed at the parking lot hit five accumulator gaps on sweep; memory consolidation sat parked until it hit the ceiling and hard-blocked new locks.*
...
**The founder, and the edge (named 2026-06-30, in his own framing):** at heart a **systems thinker** — sees the whole scaffold and how its parts norm each other. Also an **educator**, a **humanist**, and a **writing-craft person**. Those four are the root; they aren't a costume over something else. What comes out of them, when pointed at a real problem, is the studio's distinctive **edge: assessment and calibration design** — norming, scoring, rater-training, building the scaffolding that makes human judgment consistent, teachable, and reliable. The systems thinking is what makes the calibration hold together; the educator and humanist are why it serves the person rather than the metric; the craft is why it reads. (Newly recognized this session: the systems thinking is also why he can now *code* the artifacts directly.)

**The games are the delivery; the calibration is the edge underneath — and almost no one in the learning-games field is working that layer.** When a build forces a choice, the calibration edge and the humanist floor both outrank the game costume. The accessibility floors and the transparency vow make this version *trustworthy* in a way the AI-auto-scoring crowd structurally cannot claim: the algorithm is labeled, the person stays the author, the data never leaves them.

**Strengths to lean on:**
- **Systems thinking** — the root. Sees the whole scaffold and how its parts norm each other; this is what makes the calibration work and why he can code it.
- **Assessment / calibration / norming design** — the edge that the systems thinking produces. Rater calibration, anchor design, drift detection, scoring-artifact construction. Lean here first when picking what's distinctive.
- **Educator and humanist** — why the work serves the person on the other side of the screen, not the metric. The dignity-first floor lives here.
- **Writing-craft** — a genuine, distinctive creative voice (see §4): deadpan, tender, object-driven. Why the artifacts read instead of clank.
...
**The Visibility floor (locked 2026-06-27).** The founder cannot read low-contrast text. Therefore, across **all** outputs: never dark/black text on a mid-tone or gray ground; never rely on subtle tonal contrast. Readable text is near-black ink on light paper (or full inverse) at full strength. The muted-gray caption pattern is **banned** for anything meant to be read — captions included. Light-on-dark is fine when the dark ground is genuinely dark. This supersedes any house-style choice that traded contrast for elegance. Audit every build before shipping; the Reads-As, Screen-Optimization, and ADA seats each hold a HALT on it.

**The green-bail floor (locked 2026-06-30 — re-stated as a hard floor after a near-miss).** GREEN as a primary or structural color is **banned**. The founder's retinitis pigmentosa makes green-channel signal unreliable — landing on green is a bail. No green topbars, headers, primary buttons, or large fills; no green-as-state ("done"/"go") even paired with a shape, because green carries none of the load for him. Use the slate/river (blue-teal), gold, rust, and ink families instead. State is carried by **position, label, and shape** — never green. *Why it's re-locked here:* this rule lived only in old project-instructions and was never in the OS or Studio Eyes, so a freshly built file (the Studio Face, 2026-06-30) shipped green and had to be caught by the founder's own eyes — exactly the memory-dependent failure the structural-enforcement rule exists to kill. It is now an OS floor AND a Studio Eyes HALT check (green hex in any large/structural role).

**The structure floor — never land on raw text (locked 2026-06-30).** A reader (especially this one) must never land on an undifferentiated wall of text or a bare run of prose on a background with no structure to grab. Every readable surface carries structure the eye can catch first: a labeled section, a bordered card, a bar, a one-idea-per-line break, a clear hierarchy. This is the visual-rhetoric/mnemonic-design principle made a floor — text alone, with nothing structural to enter it by, is a bail the same way green is. Applies to Claude's chat replies as much as to builds: answer/move on line one, structure the rest, never a paragraph slab.

**The Provenance Floor (locked 2026-06-30 — the founder's rule, in his words: "build a system that meets my demands; floor my values").** No asset — image, text, audio, font — enters the TSP system without **recorded, legal provenance** captured *before use*. Four things are logged for every asset: **source, license, attribution string, and how-it-entered (the process note).** Hard sub-rules:
- **No scraping.** Pulling assets off the web programmatically is banned. A CC-BY *label* on a page does not prove the asset is CC-BY — re-uploads strip and fake licenses constantly. "Found online" is not a license.
- **No guessed attribution.** If provenance can't be established, the asset doesn't enter. Omit rather than invent — the same rule the research layer already follows.
- **Legal lanes only**, each tagged: CC0 / public domain (incl. the studio's own hand-drawn SVG, and verified PD sources — Wikimedia PD, Met Open Access, Smithsonian Open Access, NYPL, Wellcome); CC-BY / CC-BY-SA (used *with* the required attribution auto-generated and shown); the founder's **own art at the $1-sim purchase** (money walled from design — own work enters at a token internal price, never mixed into the design decision); commissioned (Super Sketchy Graphics, AI-assisted, standing disclosure line); licensed stock (only when actually purchased).
- **Sound is governed identically (added 2026-06-30).** Every audio asset logs source, license, attribution, and how-it-entered, before use. No scraping audio; a "free sound" label is not a license. Legal audio lanes: **synthesized-in-file** (Web Audio API generating the sound from code — CC0 by construction, no sourcing, stays single-file/offline — the *preferred default* for game SFX like the Funnyboney bonk/spill/bell); **CC0 audio** (Freesound CC0 filter, Wikimedia Commons audio, verified PD recordings); **CC-BY audio** (used with auto-generated attribution); **own/$1-sim recordings** (money-walled). Synthesized-in-file is preferred because it carries zero provenance debt and never breaks the offline floor.
- **The money wall holds here too.** The $1-sim category is how the founder's own art enters without money touching a design call — it mirrors the Scout wall (§1): provenance and price are recorded, but neither reshapes what gets made.
*Why it's a floor and not a preference:* the public GitHub site makes every asset checkable. Scraped art with guessed attribution is precisely what a funder or partner (FableVision, a grant reviewer) can point at to break the trust the whole studio runs on. The Provenance Floor protects the credibility that the moat (§2) depends on. Enforced by the Provenance Ledger tool and a Studio Eyes check (an asset reference without a provenance record is a HALT).
...
**The floors are not triggered — they are always on.** No emoji; accessibility; full-contrast visibility; scene-first opening. These don't wait for a property; they hold on everything.
...
**The publishing-layer rule (locked 2026-06-30 — the site is now real).** The studio has a live public home: a **GitHub repository** (`github.com/walshero/TIGHT-SPIRAL-STUDIOS`) served as a **GitHub Pages** site (`walshero.github.io/TIGHT-SPIRAL-STUDIOS/`). This changes the close-out chain and adds a third durability tier above the phone-save:
- **Three tiers of durability, named:** (1) *scratch* — `/mnt/user-data/outputs/`, dies with the session; (2) *the shelf* — the Project, made durable by the founder's phone-save, the working source of truth; (3) *the site* — the GitHub repo/Pages, the **public, dated, version-controlled** layer. A build isn't truly shipped until it reaches tier 3.
- **Who pushes:** Claude wires and pushes files to the repo (token-authenticated). The founder does **not** hand-manage git — the whole point of the site was to move the update burden off his plate. Close-out names the push as a move Claude makes, not a chore he inherits.
- **Why the site is the door-opener (ties to §2's moat + "evidence not vision"):** a live URL converts the founder from "professor with ideas" to "maker with a checkable portfolio." Outreach to peers (King, CUNY Games Network), grant evidence (a public dated build *is* the documented loop), and network contacts (FableVision) all run on a link, not a description. The repo is the ethos made verifiable.
- **Single-file vs. site asset (the standing fork, flagged not pre-decided):** a true offline single-file game cannot hot-link an external image URL and stay offline. When a build carries raster/MJ art, the close-out fork is: **embed** (base64 inside the file — stays single-file, file grows) **or** **co-locate** (image sits in the repo beside the HTML — no longer single-file, but works on Pages and keeps files small). The Technical Artist (§5) owns this call per build; it is decided at art-wire time, never assumed.
- **Naming still rules:** the repo is not a junk drawer. Stage-7 file discipline (name by what it IS; retire superseded files) applies to the repo exactly as to the shelf. Dead files get pruned from the repo, not just the Project.
...
- **Studio name: Tight Spiral Studios.** Tagline "Play. Notice. Design." Descriptor "We turn how you learn into how you play." Locked. (Names closed: Sandbox, Possibility Space, Spiral-alone.)
...
- **Two hard floors above all:** no emoji ever; accessibility is a design floor.
- **Visibility floor (2026-06-27):** never dark text on mid/gray ground; full-contrast ink for anything readable; muted-gray captions banned. Supersedes any elegance-over-contrast choice.
