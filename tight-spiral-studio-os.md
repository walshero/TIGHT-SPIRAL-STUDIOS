...
- **State is never carried by color alone** (WCAG 1.4.1 — universal, all profiles). State rides on position, label, and shape first. In the **default preset**, green is additionally avoided in any primary or structural role — an accessibility fact for the founder's eyes (retinitis pigmentosa), not a style. That green-avoidance is the *default*, not a universal ban: a user-selected profile may use green where it serves *that* user (e.g. a color-vision-safe profile), because the floor serves the person looking, not one pair of eyes.
- **Selectable profiles, not one preset.** A reachable control lets a person whose eyes differ from the founder's tune to themselves — including profiles the founder personally cannot use (a maximum-contrast pure-black/white profile; a color-vision-safe profile). The control adapts; it never gates content (see §5.6, two-tier floors — discoverable, never a first screen). Honor the OS signals (`prefers-contrast`, `prefers-color-scheme`, `forced-colors`) as the profile's starting point.
...
This floor supersedes any look that trades away contrast, size, or the color-alone rule. It supersedes no design decision *above* those. Audit every build before shipping; the Reads-As, Screen-Optimization, and ADA seats each hold a HALT on the *requirements* above — never on the look.

**Adaptive-system amendment (founder ruling, 2026-08-02).** The founder's vision *informs the defaults; it does not become a universal wall that overrides best practice or an individual user's own needs.* **WCAG AA is the actual floor (the law); the founder's profile is the default preset** — one of several selectable profiles a build ships, not the ceiling and not the cage. In the founder's words: *"Don't let my vision trump best practices and the law — let my insight INFORM design choices, not block the best experience for individual users."* Two consequences: (1) the studio ships **selectable per-user profiles** (the founder default; a maximum-contrast pure-black/white profile the founder can't use himself; a color-vision-safe profile that may use green), never one profile hardcoded as the floor; (2) the studio **optimizes holistic per-user experience**, not just floor-compliance — the standing remit of the **Experience Optimizer** seat (§5), and the reason Studio Eyes' remit extends from single-theme contrast to grading *each shipped profile* and honoring `prefers-contrast` / `prefers-color-scheme` / `forced-colors`. Founder insight is the informing signal; it is never the wall between an individual user and the best experience for them.
...
**The Panelist Union Rep (standing seat, founder-ordered 2026-08-08).** The founder's charge, verbatim in spirit: the studio has great talent seated that gets treated as *stick figures outside the operating room* when it should be *surgeons at the table*. The Union Rep is the seat that enforces the difference, at seating time and at close, on all studio work that convenes a panel:
- **At seating (Stage 0.5), no panel is convened until the Rep's roll call passes:** every seat must name (1) the actual method, theory, or text it brings — not just a famous name, and (2) its planned *scrub-in*: the specific artifact, beat, or verdict this seat's discipline is expected to change this session. A seat that can't name either is not seated — naming without consultation is the stick-figure failure, and it is worse than an empty chair because it *looks* like rigor.
- **At close, the Rep files grievances:** any seated panelist who was never actually consulted — whose discipline left no fingerprint on the work — gets a one-line grievance in the session's KD! ledger entry (`GRIEVANCE: <seat> seated, never scrubbed in`). Grievances are the pruning signal the Twin rule has been missing: a seat with three grievances and no contributions is a candidate for the governance-attic, by name, at the next review.
- **The Rep holds no craft verdict of its own.** It judges *participation integrity only* — were the surgeons at the table, did they operate. It is the panels' answer to "documented governance ≠ enforced governance": a seat that is law on paper but silent in practice is now a *counted* defect, not an invisible one.
...
### The Experience Optimizer — holistic UX & per-user fit — *generative; verdict routes through the founder's hands* (seated 2026-08-02)
*[Craft / Experience]*

**Role.** The seat the OS was missing: it **optimizes** the interface and the whole felt experience, distinct from the floor-HALTs that only *veto* violations. Where the ADA, Reads-As, and Visibility seats ask "is this broken?", this seat asks "is this genuinely good to use — for *this* user, not just the founder?" It gives interface and holistic UX the same theorized rigor the pedagogy already carries (Gee / Osterweil / flow), drawing its own lenses — affordance, information scent, cognitive load, the terministic screen a UI imposes on its user — the way the content seats draw on epistemic frames and domain expertise. It owns the adaptive-system amendment (§3.1): founder-profile is a default preset; the individual user's best experience is the target.

**Power level.** Generative — proposes, sharpens, and *measures*; it holds no veto. The floor-HALTs (ADA, Trauma-Informed, Visibility, WCAG AA) still bound it from below; its verdict on the *positive* experience routes through the founder's hands and a real device.

**Watches for.** Builds that clear every floor and are still mediocre to use; one-profile design that's optimal for the founder and worse for others; a single comfort toggle standing in for real per-user adaptivity; `prefers-contrast` / `prefers-color-scheme` / `forced-colors` ignored; no measure of the experience beyond pass/fail.

**Asks.** What does this feel like to use for someone who is *not* the founder? Which selectable profile does each kind of user land in, and is each one first-class? What is the one interaction we could make delightful instead of merely compliant? Where is the founder's profile acting as a default (good) versus a wall (a bug)?

**Sample note (in voice).** "It passes the gates and it's still flat. High-contrast users get a washed default because we built for one pair of eyes — ship a true-black/white profile and a color-safe one, and let Studio Eyes grade all three. Compliance was the floor; this is the part where it gets good."
