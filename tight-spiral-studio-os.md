...
**0.5-A. THE CONVENING (founder ruling, 2026-09-17 — ledger entry of that date; full spec `claude/CONVENING-HANDOFF-2026-09-17.md`).** Stage 0.5 was law and never fired unless the founder invoked it by name — the panel was a wish, and months of builds shipped past founder vision on the sensory gates alone. The ruling gives 0.5 a front door and teeth:
- **"Full TSP treatment" *means* the Convening**, invoked by saying "convene" + the build's name. Nothing is authored before it runs.
- It opens with **the sentence**: what the player does, and what the founder wants them to notice. No sentence, HALT — this is the founder-vision capture the pipeline was missing.
- **Five standing seats, blocking, every build:** Coordinator (Horvath — the cognitive work asked vs. the work named), Stranger (widened from visibility to comprehension; owns C6), **Osterweil (seated by founder ruling — the freedom-of-play test)**, Aleph, and **Studio Voice promoted to a seat** (corpus-sourced: MW Knowledge Base, sabbatical folder, the 2012 MIT and 2018 sabbatical reports; FERPA fence rides along). The Conductor sits whenever anything is seen. The per-build disciplinary bench of 0.5 extends the standing five; it does not replace them.
- **Findings land in one file** (`convening/<build>-CONVENING.md`) with a signature line per standing seat. An unsigned seat is a HALT like a failed contrast check; a draft that arrives without its findings file is out of process.
- **Check status, honestly:** the comparator that makes the findings file a required input is NOT built. Until it exists the Convening holds by discipline — the studio's known weakest material. Building it is the named next step in the handoff.
