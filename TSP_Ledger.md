...
## 2026-08-03 — Zapier auth lane: perma-fix the PAT-in-transcript gap (founder ruling)

**Founder: "I want zap to automate tokens as a studio task — I don't need stopping workflow. This is a regular gap. Perma fix." Extends the 2026-07-18 "Zapier no-PAT lane proven" entry into a standing rule.** Full node with copy-paste automation: `DECISION-zapier-auth-lane.md` (this branch).

- **DECIDED — auth source is Zapier's OAuth GitHub connection**, not hand-minted PATs. The connection (`walshero`, auto-refreshing) already writes GitHub with no token param. Stop generating fine-grained PATs that leak into the transcript and need rotation (the exact pattern `HANDOFF.md` flags). If a scoped token is ever needed it enters Zapier via the config UI out-of-band, never pasted in chat.
- **DECIDED — allowlist Zapier MCP in committed settings** so ingest/deploy actions run with no approval prompt. `.claude/settings.json` landed on the fireground lane (`1d7eee4`) and is carried on this branch. **TSP-GIT-LANE: merge it to `main`** so it loads for every session at fresh-clone start — project settings load at session start; local settings are gitignored and don't survive the clone, so canon is the only durable home.
- **WALLS DOCUMENTED (so no one re-derives)** — open-web egress (nist.gov) is policy-denied; a 34 KB image = ~46 KB base64 that an assistant cannot retype without corruption (bytes must move server-side); Zapier code-action sandboxes are single-app — GitHub-context reaches only `api.github.com`, Drive-context reads Drive but has no GitHub auth. One untested fork remains: can a Drive-context action reach `api.github.com`? That probe is step 1 next session (prompts already cleared).
- **FIREGROUND LANE, PR #28** — game scene art now labeled "studio schematic — a drawn diagram of the scene, not a photograph" (`7145161`), the honest resolution of "legal and labeled" while the raster-ingest bridge is pending. No third-party photos in the game; all art is studio cut-paper SVG.

## 2026-08-03 — Fireground: Medium Gate correction + Aleph panel kernels

**Founder ruling: "no studio created art when lives matter — actual photos." Correct, and in the OS: a documentary of real LODD incidents is the licensed-photo lane (§3.2), not cut-paper.** Studio-drawn scenes were a Medium-Gate violation and a dignity breach; removed. Convened a three-seat Aleph panel (image provenance · learning sciences · fire-domain + dignity) and worked with Funes to record the durable findings.

- **DECIDED — fireground = licensed-photo lane.** Reskinned to real-photo slots (image + mandatory source credit + dignified "placement pending" plate). Studio art removed. `reading-the-fireground.html`, fireground lane.
