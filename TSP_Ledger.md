...
- **CONNECTED** — the GitHub repo lane (`walshero/TIGHT-SPIRAL-STUDIOS`) is now reachable AND pushable from a session; founder provided a write-capable token this session. Supersedes FUNES's standing "no session can push / pull is Matt's hands." Token is transcript-exposed → founder to rotate.
